...
// function getMenuArray(response) {
//   if (Array.isArray(response)) return response;
//   if (Array.isArray(response?.results)) return response.results;
//   if (Array.isArray(response?.data)) return response.data;
//   if (Array.isArray(response?.menus)) return response.menus;
//   if (Array.isArray(response?.items)) return response.items;
//   if (Array.isArray(response?.header_menus)) return response.header_menus;
//   return [];
// }

// function normalizeMenuItem(item) {
//   const key = item?.key || item?.slug || item?.id || item?.title || item?.name;
//   const title = item?.title || item?.label || item?.name || "Menu";
//   const path = item?.path || item?.url || item?.href || "/";
//   const matchType = item?.match_type || item?.matchType || "exact";

//   return {
//     key: String(key || title).trim(),
//     title: String(title).trim(),
//     path: String(path || "/").trim() || "/",
//     match_type: String(matchType || "exact").trim(),
...
//   const matchType = normalizeText(menu?.match_type || "exact");
//   const key = normalizeText(menu?.key || "");

//   if (matchType === "custom_blog" || key === "blog") {
//     return (
...
//   if (matchType === "starts_with" || matchType === "startswith") {
...
//   const [menuFetchFailed, setMenuFetchFailed] = useState(false);
...
//     window.addEventListener("storage", handleStorage);
//     window.addEventListener("focus", handleFocus);
//     window.addEventListener("authStatusChanged", handleStorage);

//     return () => {
//       window.removeEventListener("storage", handleStorage);
//       window.removeEventListener("focus", handleFocus);
//       window.removeEventListener("authStatusChanged", handleStorage);
//     };
//   }, []);
...
//       try {
//         // Correct backend URL is: /api/headermenu/menus/
//         const response = await apiGet("/headermenu/menus/");
//         const apiMenus = getMenuArray(response)
...
//         setHeaderMenus(apiMenus);
//         setMenuFetchFailed(false);
//       } catch (error) {
//         console.error("Mobile header menu fetch error:", error);

//         if (!isMounted) return;

//         setHeaderMenus([]);
//         setMenuFetchFailed(true);
//       } finally {
//         if (isMounted) {
...
//     if (!menuFetchDone) return [];

//     // Fallback only if API fails. If API succeeds and admin disables all menus,
//     // mobile menu will correctly show no public tabs.
//     if (menuFetchFailed) {
//       return defaultMenus
//         .filter((menu) => menu.is_active)
//         .sort((a, b) => a.order - b.order);
...
//     return headerMenus;
//   }, [headerMenus, menuFetchDone, menuFetchFailed]);
...
//               <li
//                 className={`menu-item ${
//                   isMenuActive(pathname, menu) ? "current-item" : ""
//                 }`}
...
function isMenuActive(pathname, menu) {
  const currentPath = pathname || "/";
  const menuPath = normalizeHref(menu?.path || "/");
  const matchType = normalizeText(menu?.match_type || "exact");
  const key = normalizeText(menu?.key || "");
...
function closeMobileOffcanvas() {
  ...
  try {
    ...
    if (bootstrap?.Offcanvas && offcanvasElement) {
      const instance =
    ...
    // Manual fallback below will handle close cleanup.
  }
...
export default function MobileMenu() {
  ...
  const handleMobileNavigate = (event, hrefValue) => {
        ...
        <div className="mb-body">
          <ul id="menu-mobile-menu">
            {menuItems.map((menu) => (
              ...
              >
                <a
                  href={normalizeHref(menu.path)}
                  className="item-menu-mobile"
                  onClick={(event) => handleMobileNavigate(event, menu.path)}
                >
                  {menu.title}
                </a>
              </li>
