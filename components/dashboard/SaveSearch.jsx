...
export default function SaveSearch() {
  ...
  const loadSearches = async () => {
    try {
      const res = await apiGet("/admindashboard/save-searches/");
      setSearches(res);
    } catch (error) {
      console.error(error);
    }
  ...
  const deleteSearch = async (id) => {
    try {
      await apiDelete(`/admindashboard/save-searches/${id}/`);
      loadSearches();
    } catch (error) {
      alert(error.message);
    }
  ...
  return (
    <div className="main-content w-100">
      <div className="main-content-inner style-3">
        ...
        <div className="widget-box-2 wd-listing">
          <h3 className="title">My Save Searches</h3>
          <div className="tf-new-listing w-100">
            <div className="new-listing wrap-table">
              <div className="table-content">
                <div className="wrap-listing table-responsive">
                    ...
                    <tbody>
                      {searches.map((item) => (
                        <tr key={item.id}>
                          <td>{item.title}</td>
                          <td>
                            <div>{JSON.stringify(item.parameters)}</div>
                          </td>
                          <td>
                            <div>{item.email}</div>
                          </td>
                      ...
                      {searches.length === 0 && (
                        <tr>
                          <td colSpan="5">No save searches found.</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
