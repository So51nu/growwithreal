...
export default function Property() {
  ...
  const [postStatus, setPostStatus] = useState("Select");

  const loadProperties = async () => {
    try {
      const query = new URLSearchParams();
      if (search) query.append("search", search);
      if (postStatus && postStatus !== "Select") query.append("post_status", postStatus);
      const res = await apiGet(`/admindashboard/properties/?${query.toString()}`);
      setProperties(res);
    } catch (error) {
      console.error(error);
    }
  ...
  const handleDelete = async (id) => {
    try {
      await apiDelete(`/admindashboard/properties/${id}/`);
      loadProperties();
    } catch (error) {
      alert(error.message);
    }
    ...
    <div className="main-content w-100">
      <div className="main-content-inner wrap-dashboard-content">
        ...
        <div className="row">
          <div className="col-md-3">
            <fieldset className="box-fieldset">
              <label>Post Status:<span>*</span></label>
              <select
                className="form-control"
                value={postStatus}
                  ...
                  {properties.map((property) => (
                    <tr key={property.id}>
                      <td>
                        <div className="listing-box">
                          <div className="images">
                            <Image
                              alt="images"
                              src={property.imageSrc || "/images/home/house-db-1.jpg"}
                              width={615}
                              height={405}
                            />
                          </div>
                      ...
                      <td>
                        <ul className="list-action">
                          <li>
                            <Link href={`/add-property?id=${property.id}`} className="item">
                              Edit
                            </Link>
                          </li>
                          <li>
                            ...
                            >
                              {property.is_favorite ? "Unfavorite" : "Favorite"}
                            </button>
                          </li>
                          <li>
                            <button
