...
function WhatsAppIcon() {
  return (
    <svg viewBox="0 0 32 32" aria-hidden="true">
      <path
        fill="currentColor"
        d="M16.04 3C8.86 3 3.02 8.75 3.02 15.82c0 2.25.6 4.45 1.73 6.38L3 28.62l6.61-1.72a13.15 13.15 0 0 0 6.42 1.65h.01c7.18 0 13.02-5.75 13.02-12.82C29.06 8.75 23.22 3 16.04 3Zm0 23.38h-.01a10.95 10.95 0 0 1-5.58-1.52l-.4-.24-3.92 1.02 1.05-3.76-.26-.39a10.49 10.49 0 0 1-1.7-5.67c0-5.88 4.86-10.66 10.83-10.66 5.96 0 10.82 4.78 10.82 10.66 0 5.87-4.86 10.56-10.83 10.56Zm5.94-7.98c-.33-.16-1.93-.94-2.23-1.05-.3-.11-.52-.16-.74.16-.22.32-.85 1.05-1.04 1.27-.19.22-.38.24-.71.08-.33-.16-1.38-.5-2.63-1.59a9.75 9.75 0 0 1-1.82-2.24c-.19-.32-.02-.5.14-.66.15-.14.33-.38.49-.57.16-.19.22-.32.33-.54.11-.22.05-.41-.03-.57-.08-.16-.74-1.75-1.01-2.4-.27-.64-.54-.55-.74-.56h-.63c-.22 0-.57.08-.87.41-.3.32-1.15 1.11-1.15 2.7 0 1.59 1.18 3.13 1.34 3.34.16.22 2.31 3.48 5.6 4.88.78.33 1.39.53 1.87.68.79.24 1.5.21 2.07.13.63-.09 1.93-.78 2.2-1.53.27-.76.27-1.4.19-1.53-.08-.14-.3-.22-.63-.38Z"
      />
    </svg>
  );
}
...
export default function Footer1() {
  ...
  useEffect(() => {
    ...
    const positionBackToTop = () => {
      ...
      const left = isMobile ? 16 : 18;
      const bottom = isMobile ? 96 : 104;

      document.querySelectorAll(BACK_TO_TOP_SELECTOR).forEach((element) => {
    ...
    const observer = new MutationObserver(() => {
      window.requestAnimationFrame(positionBackToTop);
    });
    ...
    return () => {
      observer.disconnect();
      window.removeEventListener("resize", positionBackToTop);
    };
  }, []);
  ...
  return (
    <footer className="booking-style-footer">
      ...
      <a
        ...
        href="https://wa.me/919326183013"
        target="_blank"
        rel="noopener noreferrer"
        aria-label="Chat with Growl City Realty on WhatsApp"
      ...
      <style jsx global>{`
        ...
        /* Keep every existing back-to-top control above WhatsApp. */
        .back-to-top,
        .backtotop,
        .scroll-to-top,
        .scroll-top,
